...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path = ""  # model_data/deeplab_mobilenetv2.pth  logs/best_epoch_weights.pth
    #---------------------------------------------------------#
    ...
    with open(os.path.join(Cityscapes_path, "Segmentation/train.txt"), "r") as f:
        train_lines = f.readlines()
    with open(os.path.join(Cityscapes_path, "Segmentation/val.txt"), "r") as f:
        val_lines = f.readlines()                  # 打开指定路径的文件（"train.txt"或"val.txt"），然后读取所有的行，并将它们存储在lines列表中。

    num_train = 2975                               # 计算train_lines列表的长度，即train.txt文件中的行数，并将结果存储在num_train中
    num_val = 500
    ...
    if True:
        ...
        val_dataset     = DeeplabDataset(val_lines, input_shape, num_classes, False, Cityscapes_path)
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, Cityscapes_path, log_dir, Cuda,
